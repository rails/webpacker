...
If you are already familiar with webpack configuration, and want to see the
webpack config generated by Webpacker's environment files and webpacker.yml,
you can use the --show-config command-line option (this will also display
the internal `environment` object Webpacker generates from your config files).

```bash
  ./bin/webpacker --show-config
```
