...
Note, if you are shimming globals, you will _probably_ need to override the default of `defer: true` to `defer:false` your call to the `javascript_pack_tag` so that the browser will load your bundle to setup the global variable before other code depends on it. However, you really should try to remove the dependendency on such globals.
