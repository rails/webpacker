...
class ConfigurationTest < Minitest::Test
  def test_config_path
    config_path = File.join(File.dirname(__FILE__), "config", "webpack").to_s
    assert_equal Webpacker::Configuration.config_path.to_s, config_path
  end

  def test_entry_path
    entry_path = File.join(File.dirname(__FILE__), "app/javascript", "packs").to_s
    assert_equal Webpacker::Configuration.entry_path.to_s, entry_path
  end

  def test_file_path
    file_path = File.join(File.dirname(__FILE__), "config", "webpack", "paths.yml").to_s
    assert_equal Webpacker::Configuration.file_path.to_s, file_path
  end

  def test_manifest_path
    manifest_path = File.join(File.dirname(__FILE__), "public/packs", "manifest.json").to_s
    assert_equal Webpacker::Configuration.manifest_path.to_s, manifest_path
  end

  def test_packs_path
    packs_path = File.join(File.dirname(__FILE__), "public/packs").to_s
    assert_equal Webpacker::Configuration.packs_path.to_s, packs_path
  end

  def test_output_path
    output_path = File.join(File.dirname(__FILE__), "public").to_s
  ...
  def test_source_path
    source_path = File.join(File.dirname(__FILE__), "app/javascript").to_s
